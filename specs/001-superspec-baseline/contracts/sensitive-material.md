...
| **(Gap G3)**SSH agent socket → 容器(目前依賴 VS Code 預設) | source-control credential forwarding | 應顯式宣告於 `mounts`,使行為與 IDE 解耦 |

## 不變量(must)

1. **OAuth credentials = 0 in git**(SC-006):`gitleaks` 或等價工具掃描全 history hits = 0。
2. **永不 COPY credentials 進 image**:Dockerfile 各 stage `COPY` 命令不可包含 `~/.claude*` 或 `.credentials*`;一旦違反,該 image 屬「污染」必須重 build。
3. **`.env.example` ≠ `.env`**:範本可進 git,實際機密永不進 git。
4. **個人 override 隔離**:`.claude/settings.local.json` / `docker-compose.override.yml` 只屬個人開發機,不對 team 可見。
...
| 容器內 `claude` 看不到 host credentials | mount 路徑與 host `$HOME` 不一致(WSL 常見誤裝;不可把 repo 放 `/mnt/c/`);須檢查 `${localEnv:HOME}` 解析 |
| SSH agent forwarding 在某些 IDE 不工作 | 因 G3 未顯式化,行為依賴 IDE 預設;adopter 須自行於 `mounts` 加 `${localEnv:SSH_AUTH_SOCK} → /ssh-agent` 並設 `containerEnv.SSH_AUTH_SOCK=/ssh-agent`(Gap G3 task 內容) |

## 監控 / Audit

baseline 不規範定期 audit 頻率(視 adopter 安全成熟度而定),但建議:

- PR pre-merge:CI 跑 `gitleaks detect --redact`(可整合進 G4 CI workflow)。
