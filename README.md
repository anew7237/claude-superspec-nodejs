...
│   └── specs/               # 各 feature 的 spec / plan / tasks
├── scripts/
│   └── db-init/            # postgres 第一次啟動會跑這裡的 .sql / .sh
├── src/                     # 你的應用程式碼
│   ├── app.ts               # Hono app + routes
│   ├── index.ts             # server entry
│   ├── db.ts, redis.ts, logger.ts ... # 未全列
├── tests/                   # 測試(vitest)
...
├── CLAUDE.md                # Claude Code 的 runtime 指引(SpecKit context + Git workflow 規則)
├── docker-compose.yml       # app + db + redis 編排
├── Dockerfile               # 應用程式 image(multi-stage:dev + production)
├── eslint.config.js         # ESLint 9 flat config
├── Makefile                 # 常用指令
├── package.json             # Node 專案定義
├── pnpm-lock.yaml           # pnpm 鎖版檔(必 commit,確保跨機器相依完全一致)
├── tsconfig.json            # TypeScript 編譯設定(build 用,只含 src/)
